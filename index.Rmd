...
```{r treemap}
library(treemapify)
 p9 <- ggplot(brisbane_complaints_category, ggplot2::aes(area = n,label = category, fill = category)) + 
 geom_treemap() + 
  geom_treemap_text( stat = "identity", position = "identity", 
  na.rm = FALSE, show.legend = FALSE, inherit.aes = TRUE, 
  padding.x = grid::unit(1, "mm"), padding.y = grid::unit(1, "mm"), 
  place = "topleft", min.size = 4, grow = FALSE, 
  reflow = FALSE, fixed = NULL, layout = "squarified", start = "bottomleft") 
p9 
```
